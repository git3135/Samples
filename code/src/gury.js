window.$g = window.Gury = (function(window, jQuery) {
  ...
  Gury.prototype = {
    ...
    add: function() {
      var tag = null, object=null;
      if (arguments.length == 1) object = arguments[0];
      if (arguments.length >  1) object = arguments[1];
      if(object!=null&&isObjectOrFunction(object)) {
	      // Annotate the object with gury specific members
    	  //this.shadowColor=this.shadowColor.replace(/^\s\s*/, '').replace(/\s\s*$/, '');
    	  //if(this.shadowColor=="") this.shadowColor=null;
    	  if (!isDefined(object._gury)) {
	        object._gury = { visible: true, paused: false, z: this.nextZ(), font: this.font, fillStyle: this.fillStyle, rotation: this.rotation, opacity: this.opacity, scale: this.scale, gradient: this.gradient, pattern: this.pattern, shadowColor: this.shadowColor, shadowOffsetX: this.shadowOffsetX, shadowOffsetY: this.shadowOffsetY, shadowBlur: this.shadowBlur };
	      }
    ...
    draw: function() {
      ...
      gury._objects.each(function(ob) {
        ...
        if (typeof ob == "function") {
          ...
          ob.draw(gury.ctx, gury.canvas, ob._gury);
        }
      });
...
Shape.prototype = {
  ...
  draw: function(ctx, canvas, inst) {
      ...
      if (inst.scale) {
    	  ctx.scale(inst.scale);
      }
      if (inst.shadowColor) {
          ctx.shadowOffsetX = inst.shadowOffsetX;
          ctx.shadowOffsetY = inst.shadowOffsetY;
          ctx.shadowColor   = inst.shadowColor;
          ctx.shadowBlur    = inst.shadowBlur;
      }
      this.render(ctx, canvas, inst);
      ctx.restore();
  },
  
  
  // Put shape specific code here...
  render: function(ctx, canvas, inst) {},
...
function Rectangle() {
  Shape.apply(this, arguments);
  this.render = function(ctx, canvas, inst) {
    ctx.beginPath();
	ctx.lineWidth=this.size||1;
...
function Circle() {
  ...
  this.render = function(ctx, canvas, inst) {
    ctx.beginPath();
    ctx.lineWidth=this.size||1;
    ctx.arc(this.x, this.y, this.size||32, 0, 2*Math.PI, true);
    if (inst.fillStyle) {
        ctx.fillStyle = inst.fillStyle;
        ctx.fill();
    }
...
function Curve() {
  ...
  this.render = function(ctx, canvas, inst) {
    ctx.beginPath();
    ctx.lineWidth=this.size||1;
    ctx.bezierCurveTo(this.cx,this.cy,this.dx,this.dy,this.x,this.y);
    if (_gury.fillStyle) {
        ctx.fillStyle = inst.fillStyle;
        ctx.fill();
    }
...
function Text() {
  Shape.apply(this, arguments);
  this.render = function(ctx, canvas, inst) {
	if(inst.font) ctx.font=inst.font;
    ctx.fillText(this.text,this.x||canvas.width/2,this.y||canvas.height/2);
  };
}
...
function Picture() {
	...
	this.render = function(ctx, canvas) {
      ...
      imageObj.src = this.text; //'http://www.html5canvastutorials.com/demos/assets/darth-vader.jpg';
	};
}
Picture.prototype = new Shape();
